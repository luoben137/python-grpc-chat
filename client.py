...
class Client:
    ...
    def __init__(self, u: str, window):
        ...
        self.conn = rpc.StartxServerStub(channel)
        self.register()
    ...
    def __listen_for_messages(self):
        ...
        """
        for note in self.conn.MessageStream(
                startx_service.User(name=self.username)):  # this line will wait for new messages from the server!
            print("R[{}] {}".format(note.name, note.message))  # debugging statement
            self.startx_service_list.insert(END,
                                            "[{}] {}\n".format(note.name, note.message))  # add the message to the UI

    def register(self):
        user = startx_service.User(name=self.username)
    ...
    def send_message(self, event):
        ...
        if message != '':
            n = startx_service.Note()  # create protobug message (called Note)
    ...
    def __setup_ui(self):
        self.startx_service_list = Text()
        self.startx_service_list.pack(side=TOP)
...
if __name__ == '__main__':
    root = Tk()  # I just used a very simple Tk window for the startx_service UI, this can be replaced by anything
